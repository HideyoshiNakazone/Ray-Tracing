...
glm::vec4 Renderer::PerPixel(uint32_t x, uint32_t y)
{
    ...
    for (int i = 0; i < bounces; i++) 
    {
        ...
        float lightIntensity = glm::max(
            glm::dot(payload.WorldNormal, -lightDirection),
            0.0f
        );
        ...
        float opacityFactor = material.Roughness * .000000000075f;
        ray.Direction = glm::reflect(ray.Direction,
			payload.WorldNormal + opacityFactor * Walnut::Random::Vec3(-1.f, 1.f));
	}
